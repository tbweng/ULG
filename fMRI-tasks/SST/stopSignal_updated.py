...
test_dlg.addField('Practice or Scanner?', choices=["", "Practice", "Scanner"])
test_dlg.addField('Handedness', choices=['R','L'])
test_dlg.addField('Run 1 or Run 2?', choices=["", "1", "2"])
input_data = test_dlg.show()
expInfo = {'participant': input_data[0], 'prac_or_scan': input_data[1], 'handedness': input_data[2], 'run': input_data[3]}
...
filename = _thisDir + os.sep + u'data/%s_%s_%s_%s-%s_%s' % (expInfo['participant'], expName, expInfo['prac_or_scan'], 'run', expInfo['run'], expInfo['date'])
...
def writeTrialtoFile(trial_type, arrow, correct_response, ssd, iti, response_key, response_acc, response_rt, soa, end_fixation_duration):
   ...
   with open(file_name, 'a') as save_file:
       ...
       file_writer.writerow((trial_type, arrow, correct_response, ssd, iti, response_key, response_acc, response_rt, soa, end_fixation_duration, expInfo['participant'], trial_order, expInfo['handedness']))
...
if(expInfo['prac_or_scan'] == 'Practice'):
    trial_order = 'sst_trial_order_practice.csv'
elif(expInfo['prac_or_scan'] == 'Scanner'):
    if(int(expInfo['run']) == 1):
        randTO = str(randint(1, 12))

        # save randomized trial order to be used for run 2
        fname_TO = os.path.join(_thisDir, 'data', expInfo['participant'] + '_Scanner_run-' + expInfo['run'] + '_TO.txt')
        with open(fname_TO, 'w') as f:
            f.write(str(randTO))
            f.close()
        trial_order = 'sst_trial_order_' + randTO + 'A.csv'
    elif(int(expInfo['run']) == 2):

        # retrieve the trial order that was randomized in run 1
        fname_TO = os.path.join(_thisDir, 'data', expInfo['participant'] + '_Scanner_run-1_TO.txt')
        with open(fname_TO, 'r') as f:
            randTO = str(f.read())
        trial_order = 'sst_trial_order_' + randTO + 'B.csv'
    thisExp.addData('randomized TrialOrder', randTO)
...
#    frameRemains = 0 + (run_end_fixation/1000)- win.monitorFramePeriod * 0.75  # most of one frame period left
    frameRemains = 0 + 500 - win.monitorFramePeriod * 0.75  # most of one frame period left
